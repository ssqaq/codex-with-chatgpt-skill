...
`Codex with ChatGPT` / `codex-with-chatgpt` 是 Skill 名称，不是指定 ChatGPT。新评审默认 DeepSeek；只有本次用户另行明确要求 GPT/ChatGPT 才切换，恢复旧任务保留已保存渠道。`review resolve` / `review start` 的 `--request` 逐字使用本次用户原始要求，或从保存原文的本地文件读取后传入；不能伪造“使用 GPT 评审”或用摘要替换来改变选择。网页只收到短摘要，CLI 的原始要求不直接整段外发。

## 授权失败时先暂停，不绕开评审

1. `Codex auth token is unavailable` 表示 Codex 宿主没有提供浏览器控制授权。它不是网页未登录、额度不足或 Skill 缺依赖的证据；不要把三类问题混在一起处理。
2. 用户要求评审后修改时，授权/连接失败、`canExecute=false`、`sent=false` 或无有效回复，一律不修改业务文件或系统配置。暂停只做获准的只读诊断，保留原任务、渠道、轮次和回执；不自行取消评审、创建替代任务或改走快速流程。只有用户明确放弃评审或改变约定后，才按新范围继续。
3. 一般业务任务中不自动修改 model provider、`requires_openai_auth` 或 `auth.json`，不退出/重启 Codex，不换 Chrome、Edge 或 HTTP/CDP 通道。用户明确授权诊断修复时，先在该修复范围内检查备份、官方认证优先级和安全的本地验证，再按证据修复；不输出凭据，不改变原业务任务评审门槛。按当前宿主公开工具核实授权；需要用户操作时给出一个具体动作，不要求用户提供 token/API key。
4. 配置文件存在、依赖检查通过或网页已登录都不代表浏览器控制授权已恢复。必须实际用宿主浏览器工具成功读取页面，再核对原任务、发送回执和共识；不能补造发送证据，也不能因 `sent=true` 直接改文件。
5. Skill/CLI 不是操作系统级沙箱，不能强制拦截任意 shell。执行者每次写入前核对门槛，所有恢复、任务续接和安装操作服从更高优先级工具与权限限制。
