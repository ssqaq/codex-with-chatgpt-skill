...
2. 活跃执行期间约每 20–30 秒用内置浏览器检查原页面；两次页面相同不算卡死。不要反复调用发送或推进脚本来等待答案。
...
1. 如果评审网页、Codex 服务或浏览器动作返回 429、Too Many Requests、rate limited 等限流信号，当前轮次立即标记为“已暂停”。
2. 暂停时保存发生时间、次数和服务给出的恢复时间（如果有），关闭修改和发送权限；不会重复发送同一条消息，也不会重复创建评审会话。
3. 没有 `--retry-rate-limit` 时，`c2c review advance` 只显示状态，不调用网页脚本。恢复时间未到时，即使带了该开关也只显示状态。
4. 到达恢复时间后，带 `--retry-rate-limit` 只做一次恢复检查；再次遇到限流会把次数加一并重新暂停。恢复检查不会从第 1 轮重来。
5. 本地记录只保存固定的限流说明，不保存服务原始返回内容；真实网页回执仍需重新核对，不能把恢复检查当成已发送或已达成共识。
