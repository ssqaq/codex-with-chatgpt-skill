...
7. **补测真正的流程**：完成一组4轮和一组3轮方案评审，两组都实际修改、各跑23项测试、检查页面并做最终复核。本次调整后核心402项测试通过；每轮耗时、工具延迟和测试范围见[测试记录](docs/verification/1.19.2.md)。
8. **按30秒以内检查网页**：发出消息后马上看一次；随后以上次实际检查时间为起点，争取在30秒内再看一次。保存记录、工具运行的时间也算在内，不能忙完以后再等30秒。看到完整回复就接着处理。

9. **单次评审也能正常启动**：补齐配套脚本遗漏的智能搜索和当前模型配置；新增单次、多轮两种实际脚本入口的回归。

默认仍使用 **DeepSeek**，保持“深度思考”和“智能搜索”开启。明确说“用 GPT/ChatGPT”时才切换。**30秒说的是隔多久看一次网页，不是保证 DeepSeek 在30秒内回答完。**检查受 Codex 和浏览器工具调度影响，并非严格实时；超过30秒会记录真实延迟，发现到期就立即检查。

第二组三轮方案共约5分8秒，没有重现第一轮一直卡住。按用户确认，本版检查标准由原来的5/15秒调整为30秒以内；旧测试的时间和超时记录原样保留，不据此承诺今后每次都不会延迟。
