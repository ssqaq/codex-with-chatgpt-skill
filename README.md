...
7. **多轮评审更省时间**：第 2 轮开始只发新增内容和分歧，不重复上一轮全文；等待网页和 Codex 执行分别计时。
8. **异常会自动刹车**：429（请求太多）和 503（服务暂时不可用）会按次数退避，10 分钟没有新回复会暂停，不会重复发送或重复建会话。
9. **共识后马上接力**：双方确认后立即发起普通执行任务，不再等你重新发“继续”。
