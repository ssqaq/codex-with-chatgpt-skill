...
1. **不再选错评审渠道**：Skill 名字虽然带 ChatGPT，默认仍是 DeepSeek；你明确说用 GPT 才切换。
2. **评审失败就停住，不偷偷改配置**：没发送、没回复或门槛没通过，就不动业务文件和系统配置。取消失败也不能被旧共识重新放行。
3. **有截图也不乱换渠道**：默认由 Codex 看图并整理事实，再交给 DeepSeek 做文字评审。
4. **授权错误说清楚**：浏览器控制授权、网页登录、模型额度是不同问题，不再混为一谈。

默认仍使用 **DeepSeek**，保持“深度思考”和“智能搜索”开启。查看[本版检查范围](docs/verification/1.19.4.md)。

此前 1.19.3 的真实三轮网页等待约为24.9、29.0、23.5秒，这是**旧版历史记录，不是本版重新测出的数字**。30秒指两次检查网页之间的目标间隔，不是保证模型30秒回答完。[历史复测记录](docs/verification/1.19.3.md)。
