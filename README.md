...
1. **浏览器断开后找回原对话**：临时冻结也能进入恢复，不再因已有等待记录挡住恢复；最多自动尝试一次，限时60秒。
...
6. **补测真正的流程**：已做3轮实际方案评审、独立项目修改、14项测试、页面操作和单独最终复核，另外验证断线后找回原对话。具体耗时、复测中修复的问题与限制见[测试记录](docs/verification/1.19.1.md)。
