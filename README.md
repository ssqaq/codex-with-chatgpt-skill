...
**让 DeepSeek 或 ChatGPT 出主意，让 Codex 动手干活。**

它主要帮你做 3 件事：

1. 先让 DeepSeek 或 ChatGPT 帮你想：这个问题怎么修、功能怎么做。
...
| 功能 | 大白话说明 |
|---|---|
| 默认用 DeepSeek | 默认打开 DeepSeek 网页，自动保持“深度思考”和“智能搜索”开启 |
| 想用 GPT 就切换 | 你说“使用 GPT/ChatGPT 评审”，就走原来的 ChatGPT 流程 |
...
| 截图辅助排查 | 可以读取你明确提供的截图，帮你看界面和报错问题 |
...
它会一轮一轮显示进度，比如“多轮评审：第 1 轮”“第 2 轮”。双方意见一致以后，才会开始改代码；没达成一致之前，不会动你的文件。

## 默认用 DeepSeek，想换 ChatGPT 也行

默认让 DeepSeek 来评审，自动保持“深度思考”和“智能搜索”开启。
...
> 使用 DeepSeek 多轮评审，双方达成共识后再修改代码。每轮显示轮数、同意的地方、分歧的地方和下一步。

放心：发给 DeepSeek 或 ChatGPT 的只是简短摘要、方案和测试结果，不会把你的完整源码、完整改动记录或日志一股脑发出去。
...
它会通过只读方式读取截图来分析界面和报错，不会把图片存到 ChatGPT 的文件区，也不会上传到 GitHub。注意：图片内容经过 ChatGPT 分析时，可能会消耗你账号里的图片或消息额度。

## 完整流程

1. 你提要求。
2. DeepSeek 或 ChatGPT 想办法。
3. Codex 动手改代码。
4. 自动跑测试。
5. 再检查一遍结果。
6. 都没问题了，才告诉你完成。

如果连接失败、网页打不开或者额度用完了，它会在大约 1 分钟内停下来，告诉你卡在哪，不会一直转圈，也不会重复发送同一轮。
...
它不能凭空变出你账号里没有的模型。ChatGPT 评审时，用你账号里实际能看到的最高的那个模型和推理强度。如果你的列表里有 GPT-5.6 Sol，可以手动选它加最高强度；看不到就用你现有的最高档。
