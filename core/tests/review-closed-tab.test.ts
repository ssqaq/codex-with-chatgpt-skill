...
it('restores a proven closed tab without resetting runtime budget, rejecting unsafe or repeated evidence',()=>{
 ...
 try {
  ...
  const closedFp=createHash('sha256').update('synthetic-tab|official-chat:synthetic-session-123|https://chat.deepseek.com/a/chat/s/synthetic-session-123|closed-tab','utf8').digest('hex');
  ...
  expect(binding.lastClosedTabRestoreFingerprint).toBe(closedFp);
  expect(binding.lastClosedTabRestoreFingerprint).not.toBe(binding.lastMessageFingerprint);
  ...
  const firstSuccess=JSON.parse(fs.readFileSync(file,'utf8'));
  const sameFpState=structuredClone(firstSuccess);
  sameFpState.bindings[0].browserTabId='synthetic-tab';
  sameFpState.bindings[0].runtimeEpoch=2;
  fs.writeFileSync(file,JSON.stringify(sameFpState));
  fs.writeFileSync(evidenceFile,JSON.stringify({...evidence,capturedAt:new Date().toISOString(),oldTabId:'synthetic-tab',tabs:[{id:'restored-dup',url}]}));
  const sameFpArgs=args.map((v,i)=>args[i-1]==='-BrowserTabId'?'restored-dup':args[i-1]==='-RuntimeEpoch'?'3':v);
  expect(ps(path.join(bundle,'scripts/session_binding.ps1'),sameFpArgs).status).not.toBe(0);
  fs.writeFileSync(file,JSON.stringify(firstSuccess));
...
it('accepts cua.getTab closed-tab evidence only when inventory is unavailable and the old tab is not-found',()=>{
 ...
 try {
  ...
  const args=['-Action','RecoverRuntimeTab','-TaskId','native-smoke','-CodexThreadId','native-test-thread','-StateDir',dir,
   ...
   '-RuntimeEpoch','2','-TabMatchCount','1','-DomTargetUrl',url,'-DomSessionTitle','Synthetic test',
   '-DomMessageMarker','CODEX-BINDING-native-test-thread','-DomModel','网页当前模型（合并升级版）','-DomReasoning','深度思考','-DomSearch','智能搜索','-ClosedTabEvidenceFile',evidenceFile];
  const write=(e:object)=>{fs.writeFileSync(file,JSON.stringify(original));fs.writeFileSync(evidenceFile,JSON.stringify(e));};
  const base={source:'cua.getTab',taskId:'native-smoke',threadId:'native-test-thread',browserSurface:'codex-in-app-sidebar',capturedAt:new Date().toISOString(),oldTabId:'synthetic-tab',oldTabStatus:'not-found',inventoryStatus:'getState-unavailable',tabs:[{id:'restored',url}]};
  write({...base,oldTabStatus:'present'});
  expect(ps(path.join(bundle,'scripts/session_binding.ps1'),args).status).not.toBe(0);
  write({...base,inventoryStatus:'available'});
  expect(ps(path.join(bundle,'scripts/session_binding.ps1'),args).status).not.toBe(0);
  write(base);
  const ok=ps(path.join(bundle,'scripts/session_binding.ps1'),args);
  expect(ok.status,ok.stderr).toBe(0);
  const restored=JSON.parse(fs.readFileSync(file,'utf8')).bindings[0];
