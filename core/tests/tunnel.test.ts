...
describe("tunnel preference state", () => {
  ...
  it("finds the most recent saved choice for a new workspace", () => {
    stateDirs.push(isolateStateDir());
    chooseQuickTunnel("older");
    const newer = write(stateDirs[stateDirs.length - 1], "tunnels/newer.json", JSON.stringify({
      workspaceId: "newer",
      preference: "named",
      askedAt: "2099-01-01T00:00:00.000Z",
      configuredAt: "2099-01-01T00:00:00.000Z",
      zone: "example.com",
      tunnelName: "c2c-newer",
      hostname: "c2c-newer.example.com",
    }));
    expect(newer).toBeTruthy();
    expect(latestConfiguredTunnelState()).toMatchObject({
      workspaceId: "newer",
      preference: "named",
      zone: "example.com",
    });
