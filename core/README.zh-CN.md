...
Codex 会显示当前第几轮，把纯文字方案发给评审网页；双方确认共识后会自动开始修改，不需要再发“继续”。如果当前任务只能出方案，Skill 会在同一工作区建立普通执行任务接着做，不重新评审，也不改写原任务历史。

如果浏览器授权失效、连接返回 502/530，或 Codex/Work 额度用完，Skill 会暂停并说明原因，不会无限转圈或重复发送；恢复后从保存的轮次继续。DeepSeek 正常思考时会继续等待并按分钟回显，连续 10 分钟没有确认到新回复内容才暂停检查。
