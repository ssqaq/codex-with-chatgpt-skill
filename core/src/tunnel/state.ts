...
/** Return the most recently saved connection choice on this machine. */
export function latestConfiguredTunnelState(): TunnelState | null {
  const directory = path.join(getStateDir(), "tunnels");
  let files: string[];
  try {
  ...
  const states = files
    .map((file) => readJsonIfExists<TunnelState>(path.join(directory, file)))
    .filter((state): state is TunnelState =>
      Boolean(state && (state.preference === "quick" || state.preference === "named") && state.askedAt)
    )
    .sort((a, b) => {
      const time = (state: TunnelState): number => Date.parse(state.configuredAt ?? state.askedAt ?? "") || 0;
