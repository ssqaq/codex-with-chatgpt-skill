...
export function buildReviewMessage(s: ReviewSession): string {
  ...
  return ["[C2C]", `REVIEW_PROVIDER: ${s.reviewProvider.toUpperCase()}`, `TASK_ID: ${s.taskId}`,
    ...
    s.round > 1 ? "只评本轮修改点与尚存问题；已同意的内容不重复展开。用中文简述结论、必要建议与验收，正文不超过 600 字；摘要要求更短时按更短上限。不使用代码围栏；TASK_ID、ROUND、DECISION 各用独立一行输出，不能省略或重复。" : "用中文给出同意点、分歧、建议、测试和成功标准，正文控制在 1800 字以内；摘要要求更短时按更短上限。不使用代码围栏；TASK_ID、ROUND、DECISION 各用独立一行输出，不能省略或重复。",
