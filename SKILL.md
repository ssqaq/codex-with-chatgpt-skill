...
每一轮评审只允许使用一条整合命令完成发送准备，禁止手动拆成十几条中间命令：

1. 把完整消息正文（末尾已带绑定暗号）写入临时文件后，只调用 `send_review_round.ps1`，参数包含：TaskId、CodexThreadId、消息文件路径、轮次号、分歧总数、已解决数。
2. 脚本内部按固定顺序完成：算指纹 → 幂等键 → 绑定检查 → 发送前四项总检查（暗号、指纹、标签、绑定）→ 准备发送 → 确认发送。任何一项缺失立即报"缺什么、怎么补"，不进入半发送状态。
3. PrepareSend 之后绝不允许修改消息正文；需要改内容就从临时文件重新开始一整轮。
4. 浏览器粘贴、回车提交、回执记录仍由浏览器工具完成；脚本输出下一步精确动作，不再由人判断中间状态。

## 回执自动补记（后台小闹钟）

1. 消息提交后，每 20–30 秒检查一次页面：页面已有本轮消息但本地 `pendingReceipt=true` 时，自动调用 `RecordSendOutcome` 补记，不等卡住才救。
2. 页面没有消息且超过发送截止时间时，按原幂等键走 `RetrySendAfterTimeout`，最多重试 2 次。
3. 连续两次补记失败，回显"补记失败 + 具体原因"，进入 auditRisk 自救路径。

## 浏览器标签自动认路

1. 标签编号变化（如 7→8→9）不再是卡点：检查绑定前先按 DeepSeek 会话 URL + 页面标题 + 消息暗号三样自动识别正确标签。
2. 识别到新编号后自动调用 `RecoverRuntimeTab` 更新绑定，不需要人手动重绑。
3. 找不到匹配标签时才提示用户"右侧栏里打开原 DeepSeek 会话链接"。

## 每轮留底账（防状态损坏）

1. 每轮完成时追加写一行 JSONL 流水账到 `deepseek-review-state\audit-log.jsonl`：任务号、轮次、指纹、发送结果、分歧解决数、时间。
2. 状态文件损坏或字段缺失时，从流水账最后一行恢复轮次和绑定信息，不重发旧消息、不重开新会话。
3. 流水账只追加不修改，不记录消息正文和敏感信息。

## 分歧进度条

1. 每轮状态回显固定格式：`多轮评审：第 N 轮（已解决 X/Y 项分歧）`，X=本轮已达成一致的项数，Y=评审开始时的分歧总数。
2. 轮次记录时必须带 `-ResolvedCount` 和 `-TotalIssues`，缺参数时按"未知/未知"显示并提示补记。
3. 共识达成时回显：`多轮评审：已达成共识（X/X 项全部分歧解决）`。

## 恢复报位置

1. 评审任务恢复时必须回显：`多轮评审：从第 N 轮继续，上一轮卡在<具体字段或动作>`。
2. 卡点从状态文件的 `nextAction`、`sendPhase`、`pendingReceipt` 读取，读不到时回显"状态缺失，从流水账恢复"。
3. 恢复后不重复读文档、不重复绑定、不重发已确认的消息。

## 暂停时给对照选项

1. 连续两轮同一分歧无实质变化自动暂停时，把双方方案整理成对照表：`方案A（Codex）｜方案B（DeepSeek）｜差异点｜影响`。
2. 表格后给明确选项：`回复 A 用 Codex 方案；回复 B 用 DeepSeek 方案；回复 C 各说一次最终理由再定`。
3. 用户选择后立即按选项继续，不再重新展开全文讨论。
