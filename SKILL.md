...
5. **等待回复节奏与等待计时**：进入等待时先记下开始时间；等待期间每满 1 分钟必须更新一次回显 `多轮评审：第 N 轮（等待回复，已等待 X 分钟）`，即 3 分钟后要显示 4 分钟、4 分钟后要显示 5 分钟，不许停在旧时间。DeepSeek 的“深度思考”通常 1–5 分钟属正常，超过 5 分钟回显“等待偏久，仍在思考，未卡死”；超过 10 分钟仍无新状态，回显当前轮次、已等待时长、具体卡点后暂停，不无限轮询。每次更新之间用 20–30 秒间隔检查页面，有回复立即进入下一步。
6. **连续两次无进展就停**：`noOpReportCount>=2` 或连续两次状态完全相同时，停止重复汇报，改为显示具体卡点（哪一轮、哪个字段、下一步动作），等待用户补充信息。

## DeepSeek 发送消息必须带绑定暗号

多轮评审的每一轮消息（C1、C2、C3……）正文末尾必须单独一行附上当前任务的绑定暗号（形如 `CODEX-BINDING-<CodexThreadId>`）。原因：本地回执登记会用页面里的暗号核对消息身份；消息里没有暗号，回执就会因“session 或 marker 不一致”被拒，后续轮次全部被 auditRisk 挡住。已激活过的任务恢复时，从绑定状态读取原暗号，不要重新编造。

## auditRisk 卡住的自救路径

绑定出现 `auditRisk=true`（原因：confirmed 回执页面 session 或 marker 不一致）且消息确认已在页面存在时：

1. 用 `session_binding.ps1 -Action ForceTerminateTask` 终止当前任务（审计保留，不丢数据）。
2. 激活新任务号，用 `-Action Claim` 接回同一 DeepSeek 会话和同一标签页；Claim 会把 auditRisk 移入 previousSendAudit 并清空发送闸门。
3. 新一轮消息正文末尾必须带绑定暗号，再走 PrepareSend → ConfirmBrowserSend → 发送 → RecordSendOutcome。
4. 全程不新建 DeepSeek 会话、不新建标签页、不重发旧消息。
