...
Codex 对明确命名的 `codex-clipboard-*` 临时截图支持显式 `attachment=true` 只读
授权；除此之外不允许读取系统临时目录或任意外部路径。

读取限制：路径必须在当前工作区内（或显式授权的 Codex 临时截图），敏感文件和
越界路径拒绝，单张图片最大 10 MB，宽高不超过 8192、总像素不超过 4000 万，
同时读取数量有上限。工具会校验真实图片结构和尺寸；图片无法读取时，明确显示
失败原因并继续纯文字流程，不自动上传或切换其他视觉服务。多轮评审中每一轮仍
显示 `多轮评审：第 N 轮`，只发送图片分析摘要和评审结论，不粘贴二进制内容。
